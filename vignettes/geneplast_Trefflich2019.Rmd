...
<b>Figure S1.</b> Distribution of the inferred evolutionary roots of TFs and target genes using regulons available from the `rtni1st` data object.
...
regulon_groupnames <- c("Before","With","After")
...
labs <- c("TF-target genes rooted before the TF",
...
<b>Figure S4.</b> Distribution of the inferred evolutionary roots of TFs and TcoFs.
...
In this section we show how to calculate the OG's *abundance*, *diversity* and *plasticity*, and then map these three metrics to regulons (please, refer to @Castro2008 and @Dalmolin2011 for a detailed description). Briefly, the *abundance* metric represents the number of orthologs divided by the number of species annotated in a given OG; `abundance = 1` indicates an one-to-one relationship between the number orthologs and species, while `abundance > 1` indicates that the number of orthologs exceeds the number of species. A large abundance value suggests a large number of paralogs annotated in the OG. The *diversity* metric represents the distribution of orthologs and paralogs in the set of species annotated in a given OG; high diversity represents an homogeneous distribution, while low diversity indicates that few species concentrate most of the orthologous genes. Finally, the *plasticity* is the combination of *abundance* and *diversity* into a single metric. Low plasticity is observed in OGs of low abundance and high diversity (i.e. few orthologs distributed over many species), while high plasticity is observed in OGs of high abundance and low diversity (i.e. many orthologs concentrate on few species).
...
wilcox.test(Abundance ~ regulon_groups, data=stats_df,
            subset = regulon_groups %in% c("Before","After"))
## Wilcoxon rank sum test with continuity correction
## data:  Abundance by TRN_element
...
wilcox.test(Plasticity ~ regulon_groups, data=stats_df,
            subset = regulon_groups %in% c("Before","After"))
## Wilcoxon rank sum test with continuity correction
## data:  Plasticity by TRN_element
...
p <- ggplot(stats_df, aes(x=regulon_groups, y=Abundance, fill=regulon_groups)) + 
  ...
  scale_fill_manual(values=regulon_groupcolors[c("Before","After")]) +
...
<b>Figure S5.</b> OG's abundance (a), diversity (b) and plasticity (c) mapped to the regulon groups depicted in Figure S2.
...
<b>Figure S6.</b> Distribution of the inferred evolutionary roots of TFs and target genes using regulons available from the `rtniNormals` data object.
