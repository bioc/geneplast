...
A transcriptional regulatory network (TRN) is formed by regulators (TFs) and target genes. The `roots_df` data frame lists the evolutionary roots inferred for each TRN element, including whether the TRN element is annotated as TF or as a target.
...
For example, *CEBPG* gene is placed at root 19 while *PANX3* gene is placed at root 13. As the evolutionary roots are enumerated from the most recent to the most ancestral node of the phylogenetic tree, this indicates that the evolutionary root inferred for *CEBPG* is more ancestral than the evolutionary root inferred for *PANX3* (please note that the aim of the analysis is to find the root of the orthologs of the reference species, therefore the root enumeration is related to the ancestral lineage of the reference species; for details of the phylogenetic tree, see **Figure 4** of Geneplast's vignette).
